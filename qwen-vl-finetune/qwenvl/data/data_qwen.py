...
class LazySupervisedDataset(Dataset):
    ...
    def process_navigation_video(self, video_file):
        decord_video = None
        decord_attempts = 0
        max_decord_attempts = 3
        while decord_attempts < max_decord_attempts:
            try:
                decord_video = self.navigation_video_decord(video_file)
                return decord_video
                if decord_video:
                    break
            except Exception as e:
    ...
    def video_decord(self, video_file):
        ...
        video = vr.get_batch(frame_idx).asnumpy() # (8, 448, 448, 3)
        
        # noise ablation
        # video = self.add_noise_to_floorplan_nav(video)
        
        return self.process_video_frames(video, frame_idx, video_length)
    
    def add_gaussian_noise(self, image, mean=0, sigma=300):
    ...
    def add_noise_to_floorplan_nav(self, video):
        assert video[0].shape[1] == 448 * 2
        for frame in video:
            floorplan_nav = frame[:, 448:, :]
            noised_fp_nav = self.add_gaussian_noise(floorplan_nav)
            frame[:, 448:, :] = noised_fp_nav
        
        return video
            
    def navigation_video_decord(self, video_file):
        if not os.path.exists(video_file):
            print(f"File not exist: {video_file}")
        vr = VideoReader(video_file, num_threads=4)
        total_frames = len(vr)
        ...
        video = vr.get_batch(frame_idx).asnumpy() # (8, 448, 448, 3)
        if video_length > 1:
            history_video, history_grid_thw, history_second_per_grid_ts = self.process_video_frames(video[:-1], 
                                                                                                    frame_idx[:-1], 
                                                                                                    video_length - 1)
            current_video, current_grid_thw, current_second_per_grid_ts = self.process_video_frames(video[-1:], frame_idx[-1:], 1)
            return [history_video, current_video], [history_grid_thw, current_grid_thw], current_second_per_grid_ts 
        else:
            video, grid_thw, second_per_grid_ts = self.process_video_frames(video, frame_idx, video_length)
            return [video, video], [grid_thw, grid_thw], second_per_grid_ts
...
class LazySupervisedFloorPlanDataset(LazySupervisedDataset):
    """Dataset for supervised fine-tuning."""
    ...
    def _get_item(self, i) -> Dict[str, torch.Tensor]:
        ...
        if type(second_per_grid_ts) == tuple and len(second_per_grid_ts) == 2:
...
class LazySupervisedFloorPlanDatasetv2(LazySupervisedDataset):
    """Dataset for supervised fine-tuning."""
    ...
    def _get_item(self, i) -> Dict[str, torch.Tensor]:
        ...
        if "video" in sources[0]:
            ...
            if isinstance(video_file, List):
                ...
                video, video_grid_thw, second_per_grid_ts = self.process_navigation_video(video_file)
                # video = [video]
            video_grid_thw_merged = copy.deepcopy(video_grid_thw)
            if not isinstance(video_grid_thw, Sequence):
                video_grid_thw_merged = [video_grid_thw_merged]
            ...
            # history_grid_thw_merged = copy.deepcopy(history_grid_thw)
            # if not isinstance(history_grid_thw, Sequence):
            #     history_grid_thw_merged = [history_grid_thw_merged]
            #     history_grid_thw = [history_grid_thw]
            # history_grid_thw_merged = [
            #     merged_thw.prod() // self.data_args.image_processor.merge_size**2
            #     for merged_thw in history_grid_thw_merged
            # ]
            
            # current_grid_thw_merged = copy.deepcopy(current_grid_thw)
            # if not isinstance(video_grid_thw, Sequence):
            #     current_grid_thw_merged = [current_grid_thw_merged]
            #     current_grid_thw = [current_grid_thw]
            # current_grid_thw_merged = [
            #     merged_thw.prod() // self.data_args.image_processor.merge_size**2
            #     for merged_thw in current_grid_thw_merged
            # ]
        chat_sources = copy.deepcopy([e["conversations"] for e in sources])
        data_dict = preprocess_qwen_2_visual(
            chat_sources,
...
def make_supervised_data_module(
    ...
    train_dataset = LazySupervisedFloorPlanDataset(tokenizer=tokenizer, data_args=data_args)
