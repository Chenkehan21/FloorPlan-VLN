...
class FloorplanNavigator:
    ...
    def plot_floorplan(self, regions: List[np.ndarray], room_labels: List, 
                       traj: np.ndarray, region_ids: List[int], heading: float, scale: int=256):
        ...
        for i in range(len(traj)):
            if i + 1 <= len(traj) - 1:
                cv2.line(image, tuple(traj[i]), tuple(traj[i + 1]), color=(247,247,100), thickness=2, lineType=cv2.LINE_AA)
    ...
    def plot_floorplan_traj(self, floorplan, level_id, gt_path, heading, save_path, traj_id, step):
        ...
        region_polys, traj = self.transforms(region_polys, gt_path)
        img = self.plot_floorplan(region_polys, region_labels, traj, list(regions.keys()), heading, scale=448)
