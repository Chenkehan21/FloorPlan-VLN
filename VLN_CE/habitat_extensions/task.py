...
class VLNExtendedEpisode(VLNEpisode):
    ...
    level: Optional[Union[int, str]] = attr.ib(default=None)
    end_region: Optional[Union[int, str]] = attr.ib(default=None)
    end_region_id: Optional[Union[int, str]] = attr.ib(default=None)
    traj_regions: Optional[Union[int, str]] = attr.ib(default=None)
...
class VLNCEDatasetV1(Dataset):
    ...
    #         episode["scene_id"] = str(episode["scene_id"])
    #         episode["level"] = str(episode["level"])
    ...
    def from_json(
        self, json_str, scenes_dir: Optional[str] = None
    ) -> None:
        deserialized = json.loads(json_str)
        if isinstance(deserialized, dict):
            deserialized = deserialized["episodes"]
        for episode in deserialized:
            # cast integer IDs to strings
            episode["episode_id"] = str(episode["episode_id"])
            episode["trajectory_id"] = str(episode["trajectory_id"])
            episode["level"] = str(episode["level"])
            episode["end_region"] = str(episode["end_region"])
            episode["end_region_id"] = str(episode["end_region_id"])
            episode["traj_regions"] = episode["traj_regions"]
